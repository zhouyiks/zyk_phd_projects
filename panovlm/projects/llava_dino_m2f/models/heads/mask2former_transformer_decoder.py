# Modified from https://github.com/facebookresearch/dinov3/blob/main/dinov3/eval/segmentation/models/heads/mask2former_transformer_decoder.py
# 
...
from third_parts.dinov3.eval.segmentation.models.utils.position_encoding import PositionEmbeddingSine
...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.num_proposal_layers = num_proposal_layers
        ...
        # output FFNs
        self.class_embed = MLP(hidden_dim, hidden_dim, hidden_dim, 3)
    ...
    def forward_pre(self, x, mask_features, mask=None):
        ...
        attn_mask = self.forward_prediction_heads_only_mask(
            output, mask_features, attn_mask_target_size=size_list[0]
        )

        for i in range(self.num_proposal_layers):
    ...
    def forward_post(self, output, query_embed, text_classifier, x, mask_features, mask=None):
        # x is a list of multi-scale feature
        assert len(x) == self.num_feature_levels
        src = []
        pos = []
        ...
        outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(
            output, mask_features, attn_mask_target_size=size_list[0], text_classifier=text_classifier,
        )
        predictions_class.append(outputs_class)
        predictions_mask.append(outputs_mask)

        for i in range(self.num_proposal_layers, self.num_layers):
            level_index = i % self.num_feature_levels
            attn_mask[torch.where(attn_mask.sum(-1) == attn_mask.shape[-1])] = False
            # attention: cross-attention first
            output = self.transformer_cross_attention_layers[i](
            ...
            outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(
                output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels], text_classifier=text_classifier,
            )
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
    ...
    def forward_(self, x, mask_features, mask=None):
        # x is a list of multi-scale feature
        assert len(x) == self.num_feature_levels
        src = []
        pos = []
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, text_classifier):
        ...
        outputs_class = self.ov_class_predictor(decoder_output, text_classifier)

        return outputs_class, outputs_mask, attn_mask
    
    def forward_prediction_heads_only_mask(self, output, mask_features, attn_mask_target_size):
        decoder_output = self.post_norm(output)
        decoder_output = decoder_output.transpose(0, 1)
        mask_embed = self.mask_embed(decoder_output)
        outputs_mask = torch.einsum("bqc,bchw->bqhw", mask_embed, mask_features)
