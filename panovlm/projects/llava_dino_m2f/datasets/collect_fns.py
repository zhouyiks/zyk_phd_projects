...
def llava_dino_m2f_collate_fn(instances: Sequence[Dict],
    ...
    dinov3_inputs = []
    for example in instances:
        if example.get('dinov3_inputs', None) is not None:
            dinov3_inputs.append(example['dinov3_inputs'])
    if len(dinov3_inputs) > 0:
        data_dict['dinov3_inputs'] = torch.stack(dinov3_inputs)
    
    all_aspect_ratio = []
    for example in instances:
        if example.get('aspect_ratio', None) is not None:
            all_aspect_ratio.append(example['aspect_ratio'])
    if len(all_aspect_ratio) > 0:
        data_dict['aspect_ratio'] = torch.LongTensor(all_aspect_ratio)
