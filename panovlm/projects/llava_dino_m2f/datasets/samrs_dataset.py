...
category_dict = {
    'samrs_fast': ['Passenger-Ship', 'Dry-Cargo-Ship', 'Liquid-Cargo-Ship', 'Engineering-Ship', 'A220', 'other-airplane', 'A321', 'Small-Car', 'Van', 'Dump-Truck', 'Intersection', 'Cargo-Truck', 'Bus', 'Tugboat', 'Motorboat', 'Bridge', 'Tennis-Court', 'Baseball-Field', 'Truck-Tractor', 'Fishing-Boat', 'other-ship', 'ARJ21', 'Boeing737', 'Boeing747', 'other-vehicle', 'Boeing787', 'A330', 'Football-Field', 'Basketball-Court', 'Boeing777', 'Roundabout', 'Warship', 'C919', 'Tractor', 'Excavator', 'A350', 'Trailer'],
    'samrs_sior': ['golffield', 'expressway-toll-station', 'vehicle', 'trainstation', 'chimney', 'storagetank', 'ship', 'harbor', 'airplane', 'groundtrackfield', 'tenniscourt', 'dam', 'basketballcourt', 'expressway-service-area', 'stadium', 'airport', 'baseballfield', 'bridge', 'windmill', 'overpass'],
    'samrs_sota': ['small-vehicle', 'storage-tank', 'large-vehicle', 'plane', 'ship', 'harbor', 'tennis-court', 'swimming-pool', 'baseball-diamond', 'soccer-ball-field', 'ground-track-field', 'roundabout', 'basketball-court', 'bridge', 'helicopter', 'airport', 'container-crane', 'helipad'],
    'samrs_sota_rbb': ['Boeing777', 'C919', 'A220', 'ARJ21', 'Baseball-Field', 'Cargo-Truck', 'Bridge', 'A321', 'Boeing787', 'Basketball-Court', 'Boeing747', 'Bus', 'Boeing737', 'A350', 'A330', 'Dump-Truck', 'Dry-Cargo-Ship', 'Engineering-Ship'],
}



class SAMRSDataset(Dataset):
    ...
    def __init__(self,
        ...
        if info_file is not None and info_file != "":
            with open(info_file, 'r') as f:
                self.text_data = json.load(f)
                self.all_class_names = category_dict[dataset_name]
        else:
    ...
    def json_file_preprocess(self, data_path):
        ...
        for image_file in os.listdir(self.image_folder):
            ...
            anno_file = f"{image_id}.pkl"
            anno_path = os.path.join(data_path, anno_file)

            with open(anno_path, 'rb') as f:
                anno_dict = pickle.load(f)
                for ann in anno_dict:
                    if ann['category'] not in all_class_names:
                        all_class_names.append(ann['category'])
            data_list.append({'image_path': image_path, 'anno_path': anno_path})
    ...
    def prepare_data(self, index):
        ...
        with open(data_dict['anno_path'], 'rb') as f:
            anno_dict = pickle.load(f)

        segmentation_list, class_names = [], []
        for ann in anno_dict:
            if ann['category'] in ['background', 'unlabeled', 'unclassified', 'Background']:
                continue
            segmentation_list.append(ann['mask'])
            class_names.append(ann['category'])
        ...
        if random.random() < self.random_ratio:
            exclude_class_names = [cls_name for cls_name in self.all_class_names if not cls_name in unique_class_names]
            ...
            selected_candidate_class_names = [cls_name for cls_name in self.all_class_names if cls_name in random_select_class_names or cls_name in unique_class_names]
            selected_class_names = class_names
            selected_masks = masks
        else:
            selected_candidate_class_names = [class_name for class_name in self.all_class_names]
            selected_class_names = class_names
            selected_masks = masks
