...
class BSBInsSegDataset(Dataset):
    ...
    def json_file_preprocess(self, data_path):
        ...
        new_id_2_class_name = {}
        for k, v in id_2_class_name.items():
            if v == "comm. building block":
                new_id_2_class_name[k] = 'commercial building block'
            elif v == "comm. building":
                new_id_2_class_name[k] = 'commercial building'
            elif v == "res. building":
                new_id_2_class_name[k] = 'residential building'
            else:
                new_id_2_class_name[k] = v
        self.id_2_class_name = new_id_2_class_name
    ...
    def prepare_data(self, index):
        ...
        all_class_names = [class_name for class_id, class_name in self.id_2_class_name.items()]
        if random.random() < self.random_ratio:
            exclude_class_names = [cls_name for cls_name in all_class_names if not cls_name in unique_class_names]
            ...
            selected_candidate_class_names = [cls_name for cls_name in all_class_names if cls_name in random_select_class_names or cls_name in unique_class_names]
            selected_class_names = class_names
            selected_masks = masks
        else:
            selected_candidate_class_names = all_class_names
            selected_class_names = class_names
            selected_masks = masks
