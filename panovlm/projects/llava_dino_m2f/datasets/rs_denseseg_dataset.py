...
class RSDenseSegDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.all_cls_names = all_cls_name
        self.ins_cls_names = ins_cls_name
    ...
    def prepare_data(self, index):
        ...
        for cls_item in annotation:
            if 'class_name' in cls_item:
                ...
                cls_name = cls_item['class']
            segmentation = cls_item['segmentation']
            segmentation_list.extend(segmentation)
            class_names.extend([cls_name] * len(segmentation))
        ...
        if random.random() < self.random_ratio:
            exclude_class_names = [cls_name for cls_name in self.all_cls_names if not cls_name in unique_class_names]
            if len(exclude_class_names):
                ...
                selected_candidate_class_names = [cls_name for cls_name in self.all_cls_names if cls_name in random_select_class_names or cls_name in unique_class_names]
                selected_class_names = class_names
                selected_masks = masks
            else:
                selected_candidate_class_names = [class_name for class_name in self.all_cls_names]
                selected_class_names = class_names
                selected_masks = masks
        else:
            if random.random() < 0.5:
                random_num = random.randint(1, len(unique_class_names))
                random_select_class_names = np.random.choice(unique_class_names, random_num, replace=False)
                selected_pos_class_names = [cls_name for cls_name in self.all_cls_names if cls_name in random_select_class_names]
                
                exclude_class_names = [cls_name for cls_name in self.all_cls_names if not cls_name in unique_class_names]
                if len(exclude_class_names):
                    random_num = random.randint(1, len(exclude_class_names))
                    selected_neg_class_names = np.random.choice(exclude_class_names, random_num)
                else:
                    selected_neg_class_names = []

                selected_candidate_class_names = [cls_name for cls_name in self.all_cls_names if cls_name in selected_pos_class_names or cls_name in selected_neg_class_names]
                selected_class_names = []
                selected_masks = []
                for cls_name, mask in zip(class_names, masks):
                    if cls_name in selected_pos_class_names:
        ...
        for class_name in selected_candidate_class_names:
            if class_name in self.ins_cls_names:
                class_name_str += f"{PHRASE_START_TOKEN}{class_name}{PHRASE_END_TOKEN} {CLS_TOKEN}, "
            else:
                class_name_str += f"{PHRASE_START_TOKEN}{class_name} (semantic){PHRASE_END_TOKEN} {CLS_TOKEN}, "
        ...
        response = random.choice(ANSWER_LIST).format(seg_tokens=seg_tokens_str)
        ...
        # dinov3 inputs
        dinov3_image = Image.open(image_path).convert("RGB")
