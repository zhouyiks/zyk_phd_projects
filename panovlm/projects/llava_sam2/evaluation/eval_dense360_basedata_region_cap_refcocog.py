...
def parse_args():
    ...
    parser.add_argument(
        '--output-path',
        default='work_dirs/dense360_basedata_region_cap_pred.json',
...
class RegionCapInferenceDataset:
    ...
    def get_questions(self):
        ...
        question = "<image>\nPlease give me a short description of the region in the picture marked by <region>."
        return question
...
def main():
    ...
    for idx in tqdm.tqdm(per_rank_ids):
        ...
        try:
            prediction = None
            max_retry = 10
            while prediction is None and max_retry > 0:
                prediction = model.predict_forward(
                    question=data_batch['text'], 
                    image=data_batch['image'], 
                    mask_prompts=data_batch['mask_prompts'][0],
                    ori_image_size=(w, h),
        ...
        text_output = prediction['response']
        has_CLS = '[CLS]' in text_output
