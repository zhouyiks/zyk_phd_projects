...
from transformers import (AutoModel, AutoModelForCausalLM, AutoTokenizer,
                          ...
                          CLIPVisionModel, GenerationConfig, AutoProcessor)
...
def parse_args():
    ...
    parser.add_argument(
        '--save_dir',
        default='./work_dirs/gcg_pred_dense360vlm_detail/',
        help='save path')
    parser.add_argument(
        '--launcher',
...
IMAGE_FOLDER = '/media/home/zhouyikang/datasets/Dense-World/Sa2VA-Training/glamm_data/images/grandf/val_test'
...
class GCGInferenceDataset:
    ...
    def get_questions(self):
        question = "Could you please give me a detail description of the scene? Please respond with interleaved segmentation masks for the corresponding parts of the answer."
        return question
...
def main():
    ...
    # path = "./work_dirs/hf_sa2va_4b_sft_on_dense360"
    # model = AutoModel.from_pretrained(
    #     path,
    #     torch_dtype=torch.bfloat16,
    ...
    for idx in tqdm.tqdm(per_rank_ids):
        ...
        try:
            pred_dict = model.predict_forward(
            ...
            if w > h:
                target_h = int(500/w * h)
                target_size = (500, target_h)
            else:
                target_w = int(500/h * w)
                target_size = (target_w, 500)
            resized_image = data_batch['image'].resize(target_size)
            # try: 
            print("level 1: ", target_size)
            pred_dict = model.predict_forward(
                question=data_batch['text'], 
                image=resized_image, 
                ori_image_size=(w, h),
            ...
            # except torch.cuda.OutOfMemoryError:
            #     if w > h:
            #         target_h = int(280/w * h)
            #         target_size = (280, target_h)
            #     else:
            #         target_w = int(280/h * w)
            #         target_size = (target_w, 280)
            #     resized_image = data_batch['image'].resize(target_size)
            #     print("level 2: ", target_size)
            #     pred_dict = model.predict_forward(
            #         question=data_batch['text'], 
            #         image=resized_image, 
            #         ori_image_size=(w, h),
        ...
        if 'grounding_masks' not in pred_dict.keys() or pred_dict['grounding_masks'] is None or len(pred_dict['grounding_masks']) == 0:
            print("No SEG !!!")
            prediction['prediction_masks'] = torch.zeros((0, h, w), dtype=torch.bool)
        else:
            # prediction['prediction_masks'] = torch.stack(pred_dict['prediction_masks'], dim=0)[:, 0]
            # prediction['prediction_masks'] = torch.stack(pred_dict['grounding_masks'], dim=0)
            prediction['prediction_masks'] = torch.from_numpy(np.concatenate(pred_dict['grounding_masks'], axis=0))
            assert prediction['prediction_masks'].shape[-2] == h and prediction['prediction_masks'].shape[-1] == w
        process_and_save_output(
            args.save_dir,
            prediction['image_file'],
            pred_dict['response'],
        ...
        results.append(pred_dict['response'])
...
def process_and_save_output(output_dir, image_name, text_output, pred_masks):
    ...
    cleaned_str = cleaned_str.replace('[CLS]', '')
