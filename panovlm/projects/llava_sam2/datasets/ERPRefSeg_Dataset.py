...
REF_QUESTIONS = [
    "Please provide the segmentation mask of the entity this sentence describes: {expression}"
    "Generate a segmentation mask for the object described in the text: {expression}",
    "Produce a pixel-wise mask highlighting the entity referenced in the sentence: {expression}",
    "Create a binary mask for the target object mentioned in the context: {expression}",
    "Extract the region of interest corresponding to the entity described here: {expression}",
    "Output a segmentation map for the specific item defined in the phrase: {expression}",
    "Identify and mask the spatial boundaries of the object specified in <p>{expression}</p>.",
    "Locate the entity in the text <p>{expression}</p> and return its segmentation mask.",
    "Visualize the precise area occupied by the target described in <p>{expression}</p> as a mask.",
    "Annotate the segmentation of the object characterized in the following sentence: {expression}",
    "Isolate the pixels belonging to the entity referenced in <p>{expression}</p> using a binary mask.",
]


class ERPRefSegDataset(Dataset):
    ...
    def prepare_data(self, index):
        anno_info = self.text_data[index]

        image_file = os.path.join(self.image_folder, anno_info['image_file'])
        json_file = os.path.join(self.mask_folder, anno_info['json_file'])
        ins_id = anno_info['ins_id']
        if not os.path.exists(image_file):
            return None

        with open(json_file, 'r') as f:
            mask_caption_data = json.load(f)
        
        expression = None
        segmentation = None
        for item in mask_caption_data:
            if item['ins_id'] == ins_id:
                expression = item['brief_caption']
                segmentation = item['segmentation']
        if expression is None or segmentation is None:
            return None
        
        ori_height, ori_width = segmentation['size']
        mask = decode_mask([segmentation], ori_height, ori_width)
        
        conversation = []
        question = random.choice(REF_QUESTIONS).strip().format(expression=expression)
        ...
        # answer = f"<p>{expression}</p>{seg_tokens_str}"
        answer = f"<p>{expression}</p>{CLS_TOKEN}"
        ...
        # resized_masks = torch.nn.functional.interpolate(mask.unsqueeze(0), size=(cur_h, cur_w), mode='nearest').squeeze(0)
        ...
        # class_ids = torch.as_tensor([0]*len(padded_masks), dtype=torch.long)
