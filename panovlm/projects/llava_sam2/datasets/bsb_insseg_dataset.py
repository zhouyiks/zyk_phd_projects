...
class BSBInsSegDataset(Dataset):
    ...
    def prepare_data(self, index):
        ...
        # tiff_array = safe_read_tiff(image_file)
        ...
        # output_image.save(f'/media/home/zhouyikang/datasets/koreyoshieo/rs_seg_panoptic/bsb/visulize_masks/{image_name}.jpg')
        # exit(0)
...
class NWPUInsSegDataset(Dataset):
    os.environ['TOKENIZERS_PARALLELISM'] = 'true'
    IMG_CONTEXT_TOKEN = '<IMG_CONTEXT>'
    IMG_START_TOKEN = '<img>'
    IMG_END_TOKEN = '</img>'
    ...
    def json_file_preprocess(self, data_path):
        ...
        for img_dict, anno_dict_list in imgs_anns:
            record = {}
            record["file_name"] = os.path.join(self.image_folder, img_dict["file_name"])
            record["height"] = img_dict["height"]
            record["width"] = img_dict["width"]
            image_id = record["image_id"] = img_dict["id"]
    ...
    def prepare_data(self, index):
        ...
        # output_image = visualize(image, masks, class_names)
        # image_name = os.path.basename(image_file).split('.')[0]
        # output_image.save(f'/media/home/zhouyikang/datasets/koreyoshieo/rs_seg_panoptic/nwpu/visualize_masks/{image_name}.jpg')
        # exit(0)
...
class ISAIDInsSegDataset(Dataset):
    os.environ['TOKENIZERS_PARALLELISM'] = 'true'
    IMG_CONTEXT_TOKEN = '<IMG_CONTEXT>'
    IMG_START_TOKEN = '<img>'
    IMG_END_TOKEN = '</img>'
    ...
    def prepare_data(self, index):
        ...
        # output_image = visualize(image, masks, class_names)
        # image_name = os.path.basename(image_file).split('.')[0]
        # output_image.save(f'test_read_isaid_{image_name}.jpg')
        # exit(0)
...
class WHUMIXInsSegDataset(Dataset):
    ...
    def prepare_data(self, index):
        ...
        # process image
        if self.zhejiang:
            image_file = data_dict['file_name'].replace('.jpg', '.png')
        else:
            image_file = data_dict['file_name']
        image = Image.open(image_file).convert('RGB')
        # tiff_array = safe_read_tiff(image_file)
...
class SAMRSDataset(Dataset):
    ...
    def prepare_data(self, index):
        ...
        output_image = visualize(image, masks, class_names)
        image_name = os.path.basename(image_file).split('.')[0]
        output_image.save(f'check_xxx_{image_name}.jpg')
        exit(0)
