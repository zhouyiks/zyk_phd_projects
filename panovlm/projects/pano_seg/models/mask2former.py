...
from transformers.models.mask2former.modeling_mask2former import (
    Mask2FormerConfig, Mask2FormerMaskedAttentionDecoderLayer, 
)
class M2FRegionEncoder(nn.Module):
    ...
    def forward(
        ...
        attention_mask = nn.functional.interpolate(
            prompt_masks, size=attention_mask_target_size, mode="bilinear", align_corners=False
        )
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                attention_mask_target_size = size_list[(idx + 1) % self.num_feature_levels]
                attention_mask = nn.functional.interpolate(
                    prompt_masks, size=attention_mask_target_size, mode="bilinear", align_corners=False
                )
                attention_mask = attention_mask.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1)
