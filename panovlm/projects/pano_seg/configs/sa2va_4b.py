...
from projects.pano_seg.models.internvl import InternVL_Slowfast

from projects.pano_seg.models import VideoLLaVASAMModel
from projects.pano_seg.datasets import VideoReVOSDataset, VideoMeVISDataset, VideoRefYoutubeVOSDataset, video_lisa_collate_fn, VideoSAM2Dataset
from projects.pano_seg.datasets import VideoChatUniViDataset
from projects.pano_seg.datasets import RefCOCOgGCGDataset, OpenPsgGCGDataset, FlickrGCGDataset, GranDfGCGDataset, OspreyDataset, OspreyDescriptionDataset, OspreyShortDescriptionDataset
from projects.pano_seg.datasets import LLaVADataset
from projects.pano_seg.datasets import ReferSegmDataset
from projects.pano_seg.datasets import CoCoPanoSegDataset

from projects.pano_seg.datasets.utils.constants import (
...
CLS_SET_TOKENS = ['[CLS{id}]'.format(id=str(i).zfill(2)) for i in range(num_cls_set)]
special_tokens = ['<p>', '</p>', '<cls>', '</cls>', '[CLS]', '[BG_CLS]', '<obj>', '</obj>', '<OBJ_CONTEXT>'] + PROPOSAL_TOKENS + CLS_SET_TOKENS
