...
class LLaVADataset(Dataset):
    ...
    def _load_annotations(self, data_path, image_folder=None):
        ...
        if self.repeats < 1:
            data = random.sample(data, int(len(data) * self.repeats))
        return data
    ...
    def prepare_data(self, index):
        ...
        if data_dict.get('image', None) is not None:
            ...
            if self.arch_type == 'qwen':
                token_dict = self.get_inputid_labels_qwen2vl(data_dict['conversations'], image_token_str)
            else:
            ...
            if self.arch_type == 'qwen':
                token_dict = self.get_inputid_labels_qwen2vl(data_dict['conversations'], None)
                image = Image.fromarray(np.zeros((1024, 1024, 3), dtype=np.uint8))
                image_processor = copy.deepcopy(self.preprocessor.image_processor)
                ...
                merge_length = self.preprocessor.image_processor.merge_size ** 2
                out_data_dict['image_flags'] = torch.tensor(
                    [0] * (_data_dict['image_grid_thw'][0, 1] * _data_dict['image_grid_thw'][0, 2] // merge_length), dtype=torch.long)
                out_data_dict.update(_data_dict)
            else:
