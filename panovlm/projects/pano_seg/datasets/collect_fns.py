...
def video_lisa_collate_fn(instances: Sequence[Dict],
    ...
    has_grounding_image = any(inst.get('g_pixel_values', None) is not None for inst in instances)
    has_mask = any(inst.get('masks', None) is not None for inst in instances)
    has_bboxes = any(inst.get('bboxes', None) is not None for inst in instances)
    has_points = any(inst.get('points', None) is not None for inst in instances)
    ...
    has_prompt_mask = any(inst.get('prompt_masks', None) is not None for inst in instances)
    has_m2f_mask = any(inst.get('m2f_masks', None) is not None for inst in instances)
    ...
    for example in instances:
        ...
        if has_prompt_mask:
            if 'prompt_masks' in example.keys():
                prompt_masks.extend(example['prompt_masks'])
            if 'vp_second_per_grid_ts' in example.keys() and 'vp_grid_thw' in example.keys():
                vp_second_per_grid_ts.extend(example['vp_second_per_grid_ts'])
                vp_grid_thw.extend(example['vp_grid_thw'])
    ...
    if has_prompt_mask:
        ...
        data_dict['vp_grid_thw'] = vp_grid_thw
        data_dict['vp_second_per_grid_ts'] = vp_second_per_grid_ts
    ...
    for k in ['class_names', 'class_name_to_contiguous_id', 'class_ids']:
        k_list = []
        for example in instances:
            k_list.append(example[k] if k in example else None)
        data_dict[k] = k_list

    # TODO: collate multiple m2f_inputs into one
    m2f_pixel_values, m2f_pixel_masks = [], []
    for example in instances:
        if example.get('m2f_inputs', None) is not None:
            m2f_pixel_values.append(example['m2f_inputs']['pixel_values'])
            m2f_pixel_masks.append(example['m2f_inputs']['pixel_mask'])
    if len(m2f_pixel_masks) > 0:
        data_dict['m2f_inputs'] = {'pixel_values': torch.cat(m2f_pixel_values, dim=0),
                                'pixel_mask': torch.cat(m2f_pixel_masks, dim=0)}
        
    m2f_masks = []
    for example in instances:
        if example.get('m2f_masks', None) is not None:
            m2f_masks.append(example['m2f_masks'])
    data_dict['m2f_masks'] = m2f_masks
