...
class SAM2Config(PretrainedConfig):
    ...
    def __init__(
        ...
        num_mask_tokens: Optional[int] = None,
...
class TwoWayAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if self.skip_first_layer_pe:
            if self.is_causal:
                queries = self.self_attn(queries.permute(1, 0, 2), queries.permute(1, 0, 2), queries.permute(1, 0, 2), need_weights=False, attn_mask=attn_mask)[0]
            ...
            if self.is_causal:
                q = queries + query_pe
                attn_out = self.self_attn(q.permute(1, 0, 2), q.permute(1, 0, 2), queries.permute(1, 0, 2), need_weights=False, attn_mask=attn_mask)[0]
...
class MaskEncoderV2(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.num_mask_tokens = num_mask_tokens
        self.num_image_tokens = 64 * 64
        scale = 256 ** -0.5
        self.mask_token_positional_embedding = nn.Parameter(
            scale * torch.randn(self.num_mask_tokens, 256))
        
        self.ln_pre = nn.LayerNorm(256)
        self.ln_post = nn.LayerNorm(256)

        attn_mask = torch.zeros((self.num_image_tokens + self.num_mask_tokens, self.num_image_tokens + self.num_mask_tokens), dtype=torch.int,requires_grad=False)
        for i in range(self.num_image_tokens + self.num_mask_tokens):
    ...
    def predict_masks(
        ...
        if repeat_image:
            ...
            assert image_embeddings.shape[0] == output_tokens.shape[0], f"image_embeddings.shape: {image_embeddings.shape}, tokens.shape: {tokens.shape}"
            src = image_embeddings
        src = src + dense_prompt_embeddings
        assert (
            image_pe.size(0) == 1
        ...
        # Run the transformer
        # ==========>>>>src.shape:  torch.Size([16, 256, 64, 64])
        # ==========>>>>pos_src.shape:  torch.Size([16, 256, 64, 64])
        # ==========>>>>tokens.shape:  torch.Size([16, 32, 256])
        image_tokens = src + pos_src.to(src.dtype)
        image_tokens = image_tokens.flatten(2, 3).permute(0, 2, 1).contiguous()
        mask_tokens = output_tokens + self.mask_token_positional_embedding.unsqueeze(0).to(output_tokens.dtype)
        x = torch.cat([image_tokens, mask_tokens], dim=1)
        ...
        mask_tokens_out = x[:, -self.num_mask_tokens:, :]
        mask_tokens_out = self.ln_post(mask_tokens_out)
...
class MaskEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        attn_mask = torch.zeros((self.num_mask_tokens, self.num_mask_tokens), dtype=torch.int,requires_grad=False)
        for i in range(self.num_mask_tokens):
            attn_mask[i, i+1:] = 1
        self.register_buffer("attn_mask", attn_mask.bool())
    ...
    def predict_masks(
        ...
        # Run the transformer
        hs, src = self.transformer(src, pos_src, tokens, attn_mask=self.attn_mask)
        s = 0
    ...
    def _build_sam_heads(self):
        ...
        self.sam_mask_encoder = MaskEncoder(
            transformer=TwoWayTransformer(
                ...
                mlp_dim=2048,
                num_heads=8,
                is_causal=_get_env_bool("MASK_TOKENIZER_IS_CAUSAL", True),
            ),
            transformer_dim=self.sam_prompt_embed_dim,
            num_mask_tokens=int(os.environ.get("MASK_TOKENIZER_NUM_MASK_TOKEN", 8)),
        )
        # self.sam_mask_encoder = MaskEncoderV2(
        #     transformer_dim=self.sam_prompt_embed_dim,
        #     num_mask_tokens=int(os.environ.get("MASK_TOKENIZER_NUM_MASK_TOKEN", 8)),
        # )
        if self.use_obj_ptrs_in_encoder:
...
class VQ_SAM2(PreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        configured_num_mask_tokens = None
        configured_is_causal = None
        sam2_config = getattr(config, "sam2_config", None)
        if isinstance(sam2_config, dict):
            configured_num_mask_tokens = sam2_config.get("num_mask_tokens", None)
            configured_is_causal = sam2_config.get("is_causal", None)
        else:
            configured_num_mask_tokens = getattr(sam2_config, "num_mask_tokens", None)
            configured_is_causal = getattr(sam2_config, "is_causal", None)

        if configured_num_mask_tokens is not None:
            os.environ["MASK_TOKENIZER_NUM_MASK_TOKEN"] = str(configured_num_mask_tokens)
        if configured_is_causal is not None:
            os.environ["MASK_TOKENIZER_IS_CAUSAL"] = "1" if bool(configured_is_causal) else "0"
        
        self.model = SAM2Model._from_config(config.sam2_config)

        sam_hidden_dim = 256
        self.num_mask_tokens = int(os.environ.get("MASK_TOKENIZER_NUM_MASK_TOKEN", 8))
    ...
    @staticmethod
    def _align_tensor_to_module(tensor: torch.Tensor, module: nn.Module) -> torch.Tensor:
        if isinstance(module, nn.Identity):
            return tensor

        ref_tensor = next(module.parameters(), None)
        if ref_tensor is None:
            ref_tensor = next(module.buffers(), None)
        if ref_tensor is None:
            return tensor
    ...
    def forward_with_codes(self, pixel_values, quant_codes):
        ...
        ndim = quant_codes.ndim
        if ndim == 3:
            quant_codes = quant_codes.flatten(0, 1)
        ...
        quant_mask_embeds = self._align_tensor_to_module(
            quant_mask_embeds, self.model.sam2_model.sam_mask_decoder
        )
    ...
    def forward_with_embeds(self, pixel_values, embeds):
        ...
        embeds = self._align_tensor_to_module(embeds, self.model.sam2_model.sam_mask_decoder)
    ...
    def forward(
        ...
        mask_embeds = self._align_tensor_to_module(mask_embeds, self.concate_mask_embeds)
        ...
        quant_mask_embeds = self._align_tensor_to_module(quant_mask_embeds, self.deconcate_quant_embed)
        quant_mask_embeds = self.deconcate_quant_embed(quant_mask_embeds)
        quant_mask_embeds = quant_mask_embeds.reshape(batch_size, self.num_mask_tokens, -1).contiguous()
        quant_mask_embeds = self._align_tensor_to_module(
            quant_mask_embeds, self.model.sam2_model.sam_mask_decoder
