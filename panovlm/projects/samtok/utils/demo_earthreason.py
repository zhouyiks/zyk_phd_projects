...
def main():
    args = parse_args()
    output_dir = './eval_dataset/earthReason'
    ...
    for image_file in tqdm.tqdm(image_files):
        ...
        case_text = questions[0].strip() if questions else ""

        text_path = os.path.join(output_dir, f"{os.path.splitext(image_file)[0]}.txt")
        with open(text_path, 'w', encoding='utf-8') as f:
            f.write(case_text + '\n')

        question = "Segment with one mask: building"
        ...
        if len(quant_ids) == 0:
            ...
            try:
                ...
                continue
            output_image.save(os.path.join(output_dir, image_file.replace('.', '_pred.')))
            ...
            try:
                ...
                continue
            gt_image.save(os.path.join(output_dir, image_file.replace('.', '_gt.')))
