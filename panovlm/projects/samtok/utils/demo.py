import sys
sys.path.append("/media/disk3/zhouyikang/pano-vlm")
...
def forward(image_path, out_path, question):
    CODEBOOK_SIZE = 256
    CODEBOOK_DEPTH = 4
    NUM_MASK_TOKENS=8
    ...
    model = Qwen3VLForConditionalGeneration.from_pretrained(
        "./checkpoints/qwen3vl_4b_pretrain", torch_dtype="auto"
    ).cuda().eval()

    processor = AutoProcessor.from_pretrained("./checkpoints/qwen3vl_4b_pretrain")
...
def main():
    image_path = "/media/disk3/dataset/zipped_formatted_datasets_test_split/OEM/oem_wallace_44_1.png"
    question = "Segment with one mask: forest."
    out_path = "./demo_visualization/10.jpg"
    forward(image_path, out_path, question)
