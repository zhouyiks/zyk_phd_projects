...
def metric():
    ...
    for json_file in os.listdir('./temp_save/risbench'):
        with open(os.path.join('./temp_save/risbench', json_file), 'r') as f:
...
def main():
    ...
    txt_path = "./data/RISBench_dataset/output_phrase_val.txt"
    ...
    for case_id in tqdm.tqdm(case_ids):
        item = data[case_id]
        mask_file, expression = item
        mask_path = os.path.join('./data/RISBench_dataset/mask', mask_file)
        image_path = os.path.join('./data/RISBench_dataset/img_rgb', mask_file)
        ...
        if len(quant_ids) == 0:
            ...
            with open(f"./temp_save/risbench/{case_id}.json", 'w') as f:
