...
def metric():
    ...
    metric_dir = './temp_save/earthreason'
...
def main():
    ...
    image_files = list(os.listdir('./data/EarthReason/test/images'))
    case_ids = list(range(len(image_files)))
    for case_id in tqdm.tqdm(case_ids):
        if os.path.exists(f"./temp_save/earthreason/{case_id}.json"):
            continue
        image_file = image_files[case_id]
        image_path = os.path.join('./data/EarthReason/test/images', image_file)
        image_name = image_file.split('.')[0]

        mask_file = f'{image_name}.png'
        mask_path = os.path.join('./data/EarthReason/test/labels', mask_file)

        qa_file = f'{image_name}.json'
        qa_path = os.path.join('./data/EarthReason/test/QAs', qa_file)
        ...
        messages = [
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": "Please doing Reasoning Segmentation according to the following instruction: " + questions[0]},
                ],
            }
        ]
        ...
        if len(quant_ids) == 0:
            ...
            with open(f"./temp_save/earthreason/{case_id}.json", 'w') as f:
